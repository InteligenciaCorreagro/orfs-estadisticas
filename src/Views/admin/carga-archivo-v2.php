<?php
// src/Views/admin/carga-archivo-v2.php
// VERSIÓN 2 - SIN CACHE
ob_start();
$pageTitle = 'Cargar Archivo V2';
?>

<div class="page-header mb-3">
    <h1><i class="fas fa-file-upload"></i> Carga de Archivo Excel (V2 - Nueva Versión)</h1>
    <p class="text-muted">✨ Versión mejorada con logging detallado - Sin problemas de cache</p>
    <p style="background: #D5F4E6; padding: 10px; border-radius: 5px; border-left: 4px solid #27AE60;">
        <i class="fas fa-info-circle"></i> <strong>Log en tiempo real:</strong>
        <a href="/debug_v2.txt" target="_blank" style="color: #27AE60;">Ver debug_v2.txt</a>
    </p>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    uploadForm.addEventListener('submit', async function(e) {
        ...
        try {
            const response = await fetch('/admin/carga-archivo-v2/upload', {
                method: 'POST',
                body: formData,
                headers: {
            ...
            if (data.success) {
                ...
                uploadResult.innerHTML = `
                    ...
                    <p style="margin-top: 15px;"><a href="/debug_v2.txt" target="_blank"><i class="fas fa-file-alt"></i> Ver log detallado</a></p>
                `;
                uploadForm.reset();
                loadHistorial();
            } else {
    ...
    async function loadHistorial() {
        try {
            const response = await fetch('/admin/carga-archivo-v2/historial', {
                headers: {
                    'X-Requested-With': 'XMLHttpRequest'
                }
            });
